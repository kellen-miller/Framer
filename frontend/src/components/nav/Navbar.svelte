<script lang="ts">
    import {sticky} from '@/lib/sticky'
    import {pages} from './Pages'
    import PageLink from "./PageLink.svelte";

    export let stickToTop = true;
    let isStuck = false;

    const handleStuck = (e) => {
        return e.detail.isStuck;
    }

</script>

<section>
    {#if !stickToTop}
        <slot/>
    {/if}
    <nav class="sticky flex items-center justify-between flex-wrap bg-teal-500 p-6"
         class:isStuck
         data-position={ stickToTop ? 'top' : 'bottom' }
         on:stuck={handleStuck}
         use:sticky={ stickToTop }
    >
        <div class="flex items-center flex-shrink-0 text-white mr-6">
            <span class="font-semibold text-xl tracking-tight">Framer</span>
        </div>
        <div class="block lg:hidden">
            <button
                  class="flex items-center px-3 py-2 border rounded text-teal-200 border-teal-400 hover:text-white hover:border-white">
                <svg class="fill-current h-3 w-3" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>
                    Menu</title>
                    <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z"/>
                </svg>
            </button>
        </div>
        <div class="w-full block lg:flex lg:items-center lg:w-auto">
            {#each pages as {name, path}}
                <PageLink {name} {path}/>
            {/each}
            <div>
                <a class="inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:border-transparent hover:text-teal-500 hover:bg-white mt-4 lg:mt-0"
                   href="#">Button</a>
            </div>
        </div>
    </nav>
    {#if stickToTop}
        <slot/>
    {/if}
</section>